...
shellDIRECT = input.readline().split("\t")[0]
...
POPMAP_FILTER = input.readline().split("\t")[0]

if POPMAP_FILTER == "NULL":
	POPMAP_FILTER = POPMAP
...
shell.write("echo 'Are you ready for the script to run? (yes/no)'\nread ANSWER\nif [ $ANSWER == 'no' ]; then\n\texit 1\nfi\n\n")
...
shell.write("echo 'Did this produce the correct output? (yes/no)'\nread ANSWER\nif [ $ANSWER == 'no' ]; then\n\texit 1\nfi\n\n")
...
shell.write("echo 'Before moving on to the MAF and loci missing data filtering, YOU MAY NEED TO FILTER FOR INDIVIDUALS MISSING DATA AT THIS TIME.'\necho 'Use the file batch_" + BATCH + ".CorrectedGenotypes_biallelic.genepop to filter out individuals.'\necho 'Create a new population map file containing only the retained individuals.'\necho 'Then, when you are ready to continue, type in the filtered file name (without directory; for example, batch_1.CorrectedGenotypes_biallelic_filtered.genepop)'\nread FILENAME")


#Transpose the file for MAF calculation
shell.write("\n#Transpose the file for MAF calculation\n\n")
shell.write("echo 'Preparing for MAF filtering'\n\n")
shell.write("echo 'checking your genepop file for sample column header'\n\n")
### If necessary, add in the word "sample" as the first column header
shell.write("python " + addDIRECT + "/add_sample_to_genepop_filteredindivids.py " + stacksDIRECT + " " + BATCH + " " + "$FILENAME" + "\n\n")
### Run transpose.py
...
shell.write("python " + mafgenDIRECT  + "/genMAFfiltering.py " + POPMAP_FILTER + "\n\n")
...
shell.write("python " + mvalsDIRECT + "/genMissingLoci.py " + POPMAP_FILTER + "\n\n")
