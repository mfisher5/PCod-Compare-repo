...
################################################################################
#
# This script was altered by Mary Fisher on 4/4/2018 so that loci selected are REMOVED from the genepop file, not retained
# 
################################################################################
...
while choice1 != 'A' and choice1 != 'B':
    choice1 = raw_input("Do you want to (A) remove a random subset of loci or (B-untested) remove a specific set of loci? ")
...
if choice1 == 'A' and len(loci) > num_loci: # select a random set of loci
    ...
    for locus in loci: # 'loci' is a list of loci that exist in the genepop file
        tmp_counter += 1
        if locus not in loci_to_retain: # names of all the loci to keep ## CHANGED ON 4/4/2018
