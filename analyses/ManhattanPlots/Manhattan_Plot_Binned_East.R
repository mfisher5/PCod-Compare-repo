...
ggplot(align_data_binned, aes(x=bin, y=fst)) + 
  geom_point(size=1) +
  ggtitle("Per Locus Fst, East Pacific cod") + 
...
outliers <- read.table("batch_8_final_filtered_aligned_EAST_outliers.txt", header=TRUE)
head(outliers)
...
ggplot(align_data_binned_outliers, aes(x=bin, y=fst)) + 
  ...
  scale_colour_manual(values=c("black", "deeppink2", "deeppink4", "deeppink4"), labels=c("None", "OutFLANK", "OutFLANK+Bayescan", "OutFLANK+Bayescan"), name = "Outliers") +
  scale_size_manual(values=c(1,2,2,2), labels=c("None", "OutFLANK", "OutFLANK+Bayescan", "OutFLANK+Bayescan"), name = "Outliers") +
  ggtitle("Per Locus Fst, East Pacific cod") + 
  xlab("Atlantic cod Linkage Group") +
  ylab(expression("F"[st])) +
  theme(plot.title=element_text(hjust=0.5, size = 20)) + 
