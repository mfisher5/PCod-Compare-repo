...
align_data <- read.delim("../SlidingWindow/EastvWest/batch_8_final_filtered_aligned_SWA_input_eastwest_globalFst_sorted_cpos.txt",sep="\t",header=TRUE)
dim(align_data)
colnames(align_data)

## rename columns for R script function calls
colnames(align_data) <- c("Locus", "fst", "position")
View(align_data)
...
align_data_binned <- align_data %>%
  select(c("Locus", "fst", "position")) %>%
  mutate(bin = newbinlist)
View(align_data_binned)

align_data_binned$bin <- as.numeric(align_data_binned$bin)


# Plot binned Manhattan Plot ----------------------------------------------
ggplot(align_data_binned, aes(x=bin, y=fst)) + 
  geom_point(size=1) +
  ggtitle("Per Locus Fst, East & West Pacific cod") + 
  ...
  scale_x_continuous(limits=c(0,598315120), expand=c(0,0)) +
