...
hofile <- read.delim("East_Ho_perLocus.txt", header=TRUE, sep=" ")
head(hofile)

## join data files into a single frame. left join to fst b/c some aligned loci didn't have fst values
align_data <- left_join(hofile,infile,by="Locus")
dim(align_data)
head(align_data)
...
write.table(align_data, "batch_8_SWA_HET_input_east.txt", quote=FALSE, sep="\t")
...
write.table(align_data_sorted, "batch_8_SWA_HET_input_east_sorted.txt", quote=FALSE, sep="\t", row.names=FALSE)
...
plotting_reg_interval(Dat = align_data_sorted, Sigma_sliding_window=250000, Nb_bootstrap=100000, which.chromosome.analysis="all",which.chromosome.plot="all",division=150,name_output="batch_8_final_filtered_east_HETEROZYGOSITY",path_output=".")
...
swa_het_output <- read.delim("batch_8_final_filtered_east_HETEROZYGOSITY_kernel_smoothing_1e+05_bootstraps_sigma_250000_div150.txt")
head(swa_het_output)
plot_het(data = swa_het_output, Nb_bootstrap=100000, Nb_divisions = 150, which.chromosome.analysis="all", which.chromosome.plot="all",export = TRUE, name="plots/East/East_SWA_Het_output")
