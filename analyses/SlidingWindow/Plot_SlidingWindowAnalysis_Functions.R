...
plot_het_overlay = function(data1 = swa_het_output_east, data2 = swa_het_output_west, legend.text.data1 = "East", legend.text.data2 = "West", Nb_bootstrap=100000, Nb_divisions = 150, which.chromosome.analysis="all", which.chromosome.plot="all", average=TRUE, export = TRUE, name="SWA_Het_East_West_overlay")
{
  # subset the data if not looking at all chromosomes; otherwise, load in all data
  ...
  for (ijk in Nb_chromosome)
  {
    ...
    if(average==TRUE){
      Data1.avg = mean(Data1.part[,2], na.rm=TRUE)
      Data2.avg = mean(Data2.part[,2], na.rm=TRUE)
    }
    # set max x axis for plot by finding the last marker position
    ...
    abline(h=0)
    if(average==TRUE){
      abline(v=Data1.avg, col="red")
      abline(v=Data2.avg, col="blue")
    }
    mtext(paste(ijk),outer=TRUE,line=-2,cex=2,at=0.515)
    mtext("Map Position (bp)",outer=TRUE,side=1,line=-2,cex=2,at=0.525)
    mtext(expression(paste("H"[observed])),outer=TRUE,side=2,line=-2,cex=2,at=0.525)
...
plot_lines_overlay_divergence = function(data1 = swa_output_selection, data2 = swa_output2_selection, data3 = swa_output3_selection, Nb_divisions = 150, legend.text= c("Analysis 1","Analysis 2", "Analysis 3"), which.chromosome.analysis="all", which.chromosome.plot="all",export = TRUE, name="SWA_lines_overlay")
{
  ...
  for (ijk in Nb_chromosome)
  {
    ...
    ## plot second data set. use min/max_lim above to define the x axis
    par(new=TRUE)
    plot(Data2.part[,2], Data2.part[,3],type="l",ylim=c(0,1), xlim=c(0,max_x_lim), lwd=3, col="mediumorchid2", ylab='',xlab='',xaxt = "n", yaxt = "n")
    abline(v=selection2, col="mediumorchid2", lty = 2)
    ## plot third data set. use min/max_lim above to define the x axis
    par(new=TRUE)
    plot(Data3.part[,2], Data3.part[,3],type="l",ylim=c(0,1), xlim=c(0,max_x_lim), lwd=3, col="deepskyblue4", ylab='',xlab='',xaxt = "n", yaxt = "n")
    abline(v=selection3, col="deepskyblue4", lty = 4)
    # add title, lines to plot
    abline(h=0)
    mtext(paste(ijk),outer=TRUE,line=-2,cex=2,at=0.515)
    mtext("Map Position (bp)",outer=TRUE,side=1,line=-2,cex=2,at=0.525)
