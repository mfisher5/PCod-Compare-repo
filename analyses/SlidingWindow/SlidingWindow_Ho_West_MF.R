...
hofile <- read.delim("West_Ho_perLocus.txt", header=TRUE, sep=" ")
head(hofile)
...
write.table(align_data, "batch_8_SWA_HET_input_west.txt", quote=FALSE, sep="\t")
...
write.table(align_data_sorted, "batch_8_SWA_HET_input_west_sorted.txt", quote=FALSE, sep="\t", row.names=FALSE)
...
plotting_reg_interval(Dat = align_data_sorted, Sigma_sliding_window=250000, Nb_bootstrap=100000, which.chromosome.analysis="all",which.chromosome.plot="all",division=150,name_output="batch_8_final_filtered_west_HETEROZYGOSITY",path_output=".")
...
swa_het_output <- read.delim("batch_8_final_filtered_west_HETEROZYGOSITY_kernel_smoothing_1e+05_bootstraps_sigma_250000_div150.txt")
head(swa_het_output)
plot_het(data = swa_het_output, Nb_bootstrap=100000, Nb_divisions = 150, which.chromosome.analysis="all", which.chromosome.plot="all",export = TRUE, name="plots/West/West_SWA_Het_output")
