...
## east west data
ew_find_markers_output <- read.delim("east_SWA_Num_Name_Loci_Per_Window.txt", sep = "\t", header= FALSE, colClasses = c("character", "numeric", "numeric", "character"))
colnames(ew_find_markers_output) <- c("chromosome", "position", "num_markers", "loci_names")
View(ew_find_markers_output)

ew_find_markers_filtered <- filter(ew_find_markers_output, position %in% east_filter$position)
dim(ew_find_markers_filtered)
View(ew_find_markers_filtered)
dim(east_filter)

write.table(ew_find_markers_filtered, "east_SWA_SelectionRegions_Markers.txt", sep = "\t", 
