...
just_plot = function(data = swa_output, marker_density = output, cutoff = 2, Nb_bootstrap=100000, Nb_divisions = 150, which.chromosome.analysis="all", which.chromosome.plot="all",export = TRUE, name="SWA_output_plot")
{
  ...
  for (ijk in Nb_chromosome)
  {
    ...
    # subset marker data to only include that chromosome and save as separate data frame
    marker_density_subset = subset(marker_density, chrom %in% ijk)
    # apply cutoff #
    marker_density_subset = subset(marker_density, num_markers >= cutoff)
    Data.part
    ...
    # save column "num_markers" as vector for plotting.
    ...
    # if you want to explore, write out the plot 
    if( export == TRUE ){
      dev.off()
    }
    count = count + 1
...
##-- plot
just_plot(data = sla_output, marker_density = output, Nb_bootstrap=100000, which.chromosome.analysis="all", which.chromosome.plot="all",name="batch_8_eastwest_output_c2")
?par
