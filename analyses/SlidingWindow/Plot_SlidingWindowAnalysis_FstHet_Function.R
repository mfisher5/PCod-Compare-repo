plot_fst_het = function(data = swa_output,
  ...
  for (ijk in Nb_chromosome)
  {
    ...
    # find average heterozygosity
    if(average==TRUE){
      Het1.avg = mean(Het1.part[,3], na.rm=TRUE)
      Het2.avg = mean(Het2.part[,3], na.rm=TRUE)
      print(Het1.avg)
    }
    ...
    plot(Data.part[,2], Data.part[,3],type="l",ylim=c(0,1.0), xlim=c(0,max_x_lim), lwd=3, col="black", ylab='',xlab='',xaxt = 'n', cex.lab=1, cex.axis=1)
    ...
    plot(Het1.part[,2], Het1.part[,3],type="l",ylim=c(0,0.5), xlim=c(0,max_x_lim), lwd=3, col="darkorchid2", ylab='',xlab='',cex.lab=1, cex.axis=1, yaxs="i")
    ## plot second data set. use min/max_lim above to define the x axis
    par(new=TRUE)
    plot(Het2.part[,2], Het2.part[,3],type="l",ylim=c(0,0.5), xlim=c(0,max_x_lim), lwd=3, col="deepskyblue4", ylab='',xlab='',xaxt = "n", yaxt = "n", yaxs = "i")
    # add title, lines to plot
    if(average==TRUE){
      abline(h=Het1.avg, col="darkorchid2", lty=2)
      abline(h=Het2.avg, col="deepskyblue4", lty=2)
    }
    abline(h=0)
    mtext(paste(ijk),outer=TRUE,line=-2,cex=2,at=0.515)
    mtext("Map Position (bp)",outer=TRUE,side=1,line=-2,cex=2,at=0.525)
    ...
    legend("topright", legend=c(legend.text.het1, legend.text.het2), fill = c("darkorchid2", "deepskyblue4"), bty = "n", cex = 1)
    # write out the plot 
    if( export == TRUE ){
      dev.off()
    }
...
# With Confidence Intervals for Heterozygosity ----------------------------
plot_fst_het_ci = function(data = swa_output,
                        het1 = swa_het_output_east,
                        het2 = swa_het_output_west,
                        legend.text.het1 = "East", legend.text.het2 = "West",
                        Nb_bootstrap_fst=100000, Nb_divisions_fst = 150, 
  ...
  for (ijk in Nb_chromosome)
  {
    ...
    par(mar=c(5,4,0,2) + 0.1)
    plot(Het1.part[,2], Het1.part[,3],type="l",ylim=c(0,0.5), xlim=c(0,max_x_lim), lwd=3, col="darkorchid2", ylab='',xlab='',cex.lab=1, cex.axis=1, yaxs="i")
    lines(Het1.part$position, Het1.part$lower_95, col=rgb(0.178,0.058,0.283))
    lines(Het1.part$position, Het1.part$upper_95, col=rgb(0.178,0.058,0.283))
    if(length(where.NA1)!=0) polygon(c(Het1.part$position[-where.NA1], rev(Het1.part$position[-where.NA1])), c(Het1.part$lower_95[-where.NA1], rev(Het1.part$upper_95[-where.NA1])), col=rgb(0.178,0.058,0.283,0.25))
    if(length(where.NA1)==0) polygon(c(Het1.part$position, rev(Het1.part$position)), c(Het1.part$lower_95, rev(Het1.part$upper_95)), col=rgb(0.178,0.058,0.283,0.25))
    ## plot second data set. use min/max_lim above to define the x axis
    par(new=TRUE)
    plot(Het2.part[,2], Het2.part[,3],type="l",ylim=c(0,0.5), xlim=c(0,max_x_lim), lwd=3, col="deepskyblue4", ylab='',xlab='',xaxt = "n", yaxt = "n", yaxs = "i")
    lines(Het2.part$position, Het2.part$lower_95, col=rgb(0,0.104,0.139))
    lines(Het2.part$position, Het2.part$upper_95, col=rgb(0,0.104,0.139))
    if(length(where.NA2)!=0) polygon(c(Het2.part$position[-where.NA2], rev(Het2.part$position[-where.NA2])), c(Het2.part$lower_95[-where.NA2], rev(Het2.part$upper_95[-where.NA2])), col=rgb(0,0.104,0.139,0.25))
    if(length(where.NA2)==0) polygon(c(Het2.part$position, rev(Het2.part$position)), c(Het2.part$lower_95, rev(Het2.part$upper_95)), col=rgb(0,0.104,0.139,0.25))
    # add title, lines to plot
    if(average==TRUE){
      abline(h=Het1.avg, col="darkorchid2", lty=2)
      abline(h=Het2.avg, col="deepskyblue4", lty=2)
