...
fstfile <- read.delim("batch_8_final_filtered_aligned_genepop_eastwest_parsed.txt",sep="\t", header=TRUE)
head(fstfile)
...
write.table(align_data, "batch_8_final_filtered_aligned_SWA_input_eastwest.txt", quote=FALSE, sep="\t")
...
write.table(align_data_sorted, "batch_8_final_filtered_aligned_SWA_input_eastwest_sorted.txt", quote=FALSE, sep="\t", row.names=FALSE)
...
plotting_reg_interval(Dat = align_data_sorted, Sigma_sliding_window=250000, Nb_bootstrap=100000, which.chromosome.analysis="all",which.chromosome.plot="all",division=150,name_output="batch_8_SWA_eastwest_output",path_output=".")
