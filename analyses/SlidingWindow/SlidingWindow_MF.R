...
#
# MF edited for Pacific cod 2/13/2018
#
###############################################
...
## join data files into a single frame
align_data <- left_join(infile,fstfile,by="Locus")
dim(align_data)
colnames(align_data)
...
write.table(align_data, "batch_8_MB_SLA_input.txt", quote=FALSE, sep="\t", row.names=FALSE)
...
View(align_data_sorted)
write.table(align_data_sorted, "batch_8_MB_SLA_input_sorted.txt", quote=FALSE, sep="\t", row.names=FALSE)
...
# RUN SLA on LG01 (test - sliding window 150kb) ---------------------------------------------------------

## set graphical parameters
#par(mfrow=c(6,6), mar=c(2,2,2,2))

## run sliding window analysis function
plotting_reg_interval(Dat = align_data_sorted, Sigma_sliding_window=150000, Nb_bootstrap=1000000, which.chromosome.analysis="LG01",which.chromosome.plot="LG01",division=200,name_output="batch_8",path_output=".")

## Compare to Manhattan Plot
library(ggplot2)
LG01_df <- filter(align_data_sorted, chromosome == "LG01")
...
# RUN SLA on LG01 (test - sliding window 200kb) ---------------------------------------------------------

## set graphical parameters
#par(mfrow=c(6,6), mar=c(2,2,2,2))

## run sliding window analysis function
plotting_reg_interval(Dat = align_data_sorted, Sigma_sliding_window=200000, Nb_bootstrap=1000000, which.chromosome.analysis="LG01",which.chromosome.plot="LG01",division=200,name_output="batch_8",path_output=".")
...
# RUN SLA on LG01 (test - sliding window 250kb, divisions = 150) ---------------------------------------------------------

## set graphical parameters
#par(mfrow=c(6,6), mar=c(2,2,2,2))

## run sliding window analysis function
plotting_reg_interval(Dat = align_data_sorted, Sigma_sliding_window=250000, Nb_bootstrap=1000000, which.chromosome.analysis="LG01",which.chromosome.plot="LG01",division=150,name_output="batch_8",path_output=".")
...
plotting_reg_interval(Dat = align_data_sorted, Sigma_sliding_window=250000, Nb_bootstrap=100000, which.chromosome.analysis="all",which.chromosome.plot="all",division=150,name_output="batch_8",path_output=".")


  
# Run across Linkage Groups  
## import new dataset with cumulative positions; "chromosome" should all = 1
cpos_data <- read.delim("batch_8_MB_SLA_input_cpos_genome.txt",header=TRUE)
View(cpos_data)
plotting_reg_interval(Dat = cpos_data, Sigma_sliding_window=200000, Nb_bootstrap=1000000, which.chromosome.analysis="PCodG",which.chromosome.plot="PCodG",division=3450,name_output="batch_8",path_output=".")
